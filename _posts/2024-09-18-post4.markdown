...
Beta, for a right tailed test as in the image above, is therefore calculated as:
...
Note that for a two-tailed test, the formula for beta changes to:

$$
\beta = P\left(z < z_{\alpha/2} - z_{\text{MDE}}\right) + P\left(z > - z_{\alpha/2} + z_{\text{MDE}}\right)
$$
...
As before, the z-statistic is calculated by dividing the observed difference by the standard error, to measure how many standard errors the observed difference is from the expected difference under the Null Hypothesis. Below is the formula for the Statistical Power of a right-tailed test.
...
The corresponding formula for Statistical Power in the case of the difference in proportions is fundamentally similar to that in the case of the difference in means:
...
\text{Statistical Power} = 1 - \beta = 1 - (P\left(z < z_{\alpha/2} - z_{\text{MDE}}\right) + P\left(z > -z_{\alpha/2} + z_{\text{MDE}}\right))
...
In the section for z-test, we have seen that the formula for statistical power for a right tailed test = \\(1 - P(Z < Z_{\alpha} - Z_{\text{MDE}}) = P(Z < Z_{\text{MDE}} - Z_{\alpha})\\)

For a right tailed test, the critical value of \\(Z_{\alpha}\\) for a 5% significance level is 1.96.
...
On the other hand, the formula for statistical power for a two tailed test = \\(1 - (P(Z < Z_{\alpha/2} - Z_{\text{MDE}}) + P(Z > -Z_{\alpha/2} + Z_{\text{MDE}}))\\) = P(Z > Z_{\text{MDE} - Z_{\alpha/2}) + P(Z < Z_{\alpha/2} - Z_{\text{MDE})
